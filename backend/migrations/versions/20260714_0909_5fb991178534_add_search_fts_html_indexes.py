...
from app.domains.system.search.indexes import HTML_FTS_INDEXES, create_index_statement
...
def upgrade():
    for index in HTML_FTS_INDEXES:
        op.execute(create_index_statement(index))


def downgrade():
    for index in reversed(HTML_FTS_INDEXES):
